...
public class SportStepJsonUtils {
    ...
    public static final String TODAY = DBConstants.TODAY;

    static JSONArray getSportStepJsonArray(List<StepData> todayStepDataArrayList) {
        JSONArray jsonArray = new JSONArray();
        if (null == todayStepDataArrayList || 0 == todayStepDataArrayList.size()) {
            return jsonArray;
        }
        for (int i = 0; i < todayStepDataArrayList.size(); i++) {
            StepData todayStepData = todayStepDataArrayList.get(i);
            try {
    ...
    static JSONObject getJSONObject(StepData todayStepData) throws JSONException {
